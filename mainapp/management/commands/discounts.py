...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        action_3_condition = Q(order__update_at__lte=F('order__created_at') + action_2_delta)
        ...
        for item in orders_items_list:
            print(f'{item.action_order:3}: заказ №{item.pk:3}: {item.product.name}: {item.discount_price}: '
